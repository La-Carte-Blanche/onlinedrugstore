...
import { removeFromCart } from "../redux/slices/cartSlice"; // import the action
...
function PlaceOrderScreen({ history }) {
  ...
  useEffect(() => {
    const prescriptionRequiredItem = cart.cartItems.find(item => item.prescriptionRequired);
    if (prescriptionRequiredItem) {
  ...
  const placeOrder = () => {
    if (isPrescriptionRequired && !prescriptionImage) {
      alert('Prescription image is required');
      return;
    }
  ...
  return (
    <div>
      ...
      <h3>Please upload your prescription here</h3>
      <input type="file" id="prescription_image" name="prescription_image" onChange={handleImageUpload} />
